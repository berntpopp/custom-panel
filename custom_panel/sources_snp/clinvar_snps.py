...
def fetch_clinvar_snps(config: dict, gene_panel: Optional[pd.DataFrame] = None) -> pd.DataFrame:
    """
    ...
    try:
        ...
        deep_intronic_variants = _filter_deep_intronic_variants(panel_filtered_variants, panel_genes, clinvar_config)
...
def _get_clinvar_vcf_file(config: dict) -> Optional[Path]:
    """
...
def _download_clinvar_vcf(url: str, cache_dir: Path, cache_expiry_days: int) -> Optional[Path]:
    """
    ...
    if cache_file.exists():
        ...
        if file_age.days < cache_expiry_days:
            ...
            logger.info(f"Cached ClinVar VCF is {file_age.days} days old, re-downloading...")
        ...
        with open(cache_file, 'wb') as f:
            for chunk in response.iter_content(chunk_size=8192):
                if chunk:
                    ...
                    if downloaded_size > 0 and downloaded_size % (50 * 1024 * 1024) < 8192:
                        if total_size > 0:
...
def _parse_clinvar_vcf(vcf_file: Path, target_chromosomes: set = None) -> pd.DataFrame:
    """
    ...
    with open_func(vcf_file, mode) as f:
        for line_num, line in enumerate(f, 1):
            ...
            try:
                ...
                if target_chromosomes and _standardize_chromosome(chrom) not in target_chromosomes:
                    continue
            ...
            if len(variants) % 100000 == 0 and len(variants) > 0:
                ...
                if len(variants) >= 2000000:  # 2M variants should be sufficient for panel genes
...
def _filter_pathogenic_variants(variants_df: pd.DataFrame) -> pd.DataFrame:
    ...
    pathogenic_text_mask = variants_df['clnsig'].str.contains(
        r'\bPathogenic\b|\bLikely_pathogenic\b',
        case=False,
        na=False,
        regex=True
    )
    ...
    logger.info(f"Filtered to {len(filtered_df)} pathogenic variants from {len(variants_df)} total variants")
...
def _convert_to_snp_format(variants_df: pd.DataFrame) -> pd.DataFrame:
    ...
    Returns:
        DataFrame in standardized SNP format
    """
    if variants_df.empty:
        return pd.DataFrame()
    ...
    # Create standardized SNP data with coordinates already included
    snp_data = pd.DataFrame({
        'rsid': variants_df['rsid'],
        ...
        # Add coordinate information to avoid Ensembl lookup
        'hg38_chr': variants_df['chromosome'].apply(_standardize_chromosome),
        'hg38_pos': variants_df['position'],
        'hg19_chr': None,  # ClinVar VCF is GRCh38, no hg19 coords available
        'hg19_pos': None,
    })

    # Keep variants with valid positions (rsID not required for ClinVar)
    snp_data = snp_data.dropna(subset=['position'])

    # Create unique IDs for variants without rsIDs
    missing_rsid_mask = snp_data['rsid'].isna()
    if missing_rsid_mask.any():
        snp_data.loc[missing_rsid_mask, 'rsid'] = (
            'clinvar_' +
            snp_data.loc[missing_rsid_mask, 'chromosome'].astype(str) + '_' +
            snp_data.loc[missing_rsid_mask, 'position'].astype(str)
        )
...
def _filter_variants_by_gene_panel(variants_df: pd.DataFrame, panel_genes: pd.DataFrame, config: dict) -> pd.DataFrame:
    """
    ...
    if not all(col in panel_genes.columns for col in required_coords):
        logger.warning(f"Gene panel missing required coordinate columns {required_coords}")
        logger.warning(f"Available columns: {list(panel_genes.columns)}")
        return pd.DataFrame()

    logger.info(f"Filtering {len(variants_df)} variants against {len(panel_genes)} genes using optimized algorithm")

    # Prepare variants with standardized chromosomes and gene symbols
    variants_prepared = variants_df.copy()
    variants_prepared['std_chr'] = variants_prepared['chromosome'].apply(lambda x: _standardize_chromosome(str(x)))
    variants_prepared['gene_symbol'] = variants_prepared['geneinfo'].apply(_extract_gene_symbol)

    # Prepare genes with standardized chromosomes
    genes_prepared = panel_genes.copy()
    genes_prepared['std_chr'] = genes_prepared['chromosome'].apply(lambda x: _standardize_chromosome(str(x)))
    ...
    for chrom in genes_prepared['std_chr'].unique():
        ...
        logger.debug(f"Processing {len(chrom_variants)} variants on {chrom} against {len(chrom_genes)} genes")
    ...
    logger.info(f"Filtered to {len(result_df)} variants within {len(panel_genes)} gene panel regions")

    return result_df


def _filter_deep_intronic_variants(variants_df: pd.DataFrame, panel_genes: pd.DataFrame, config: dict) -> pd.DataFrame:
    """
    ...
    logger.info("Note: Using simplified intronic filtering - variants within gene boundaries are considered potentially intronic")
    ...
    logger.info(f"Identified {len(deep_intronic_variants)} potential deep intronic variants")
