...
def _filter_variants_by_gene_panel_vectorized(
    ...
    variants_df["std_chr"] = variants_df["chromosome"].apply(
        lambda x: _standardize_chromosome(str(x))
    )
    variants_df["gene_symbol"] = variants_df["geneinfo"].apply(_extract_gene_symbol)

    panel_genes["std_chr"] = panel_genes["chromosome"].apply(
        lambda x: _standardize_chromosome(str(x))
    )

    # Convert coordinates to numeric
    panel_genes["gene_start"] = pd.to_numeric(
        panel_genes["gene_start"], errors="coerce"
    )
    ...
    for chrom in panel_genes["std_chr"].unique():
        ...
        within_gene = (variant_positions >= gene_starts) & (
            variant_positions <= gene_ends
        )
...
def _filter_deep_intronic_vectorized(
    ...
    logger.info(
        f"Applying vectorized deep intronic filter with {intronic_padding}bp padding"
    )
                ...
                all_exon_intervals.append(
                    {
                        ...
                        "chromosome": _standardize_chromosome(
                            str(exon.get("chromosome", ""))
                        ),
    ...
    with ProcessPoolExecutor(max_workers=n_cores) as executor:
        ...
        for batch in chrom_batches:
            ...
            batch_genes = panel_genes[
                panel_genes["chromosome"]
                .apply(lambda x: _standardize_chromosome(str(x)))
                .isin(batch)
            ].copy()
...
def _process_chromosome_batch(
    ...
    for idx, variant in variants_df.iterrows():
        ...
        # variant_gene = variant["gene_symbol"]  # Unused in this implementation

        # Find overlapping genes
        gene_matches = panel_genes[
            (
                panel_genes["chromosome"].apply(
                    lambda x: _standardize_chromosome(str(x))
                )
                == variant_chr
            )
