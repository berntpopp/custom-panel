        ...
        try:
            ...
            if source_column in df.columns:
                ...
                if len(source_groups) == len(rsids):
                    ...
                    logger.warning(f"Source column '{source_column}' length doesn't match rsID column")
            
            logger.info(f"Successfully parsed {len(result_df)} rsIDs from {self.file_path}")
...
class PDFRSIDParser(BaseSNPParser):
    ...
    def parse(self) -> pd.DataFrame:
        ...
        try:
            ...
            logger.info(f"Successfully parsed {len(df)} unique rsIDs from PDF {self.file_path}")
...
class HTMLRSIDParser(BaseSNPParser):
    ...
    def parse(self) -> pd.DataFrame:
        ...
        try:
                ...
                try:
                    ...
                    if df_list:
                        ...
                        if rsid_column in df.columns:
                            ...
                            rsids.extend([str(rsid) for rsid in table_rsids if str(rsid).startswith('rs')])
                        else:
                            # Search all columns for rsIDs
                            for col in df.columns:
                                col_data = df[col].dropna().astype(str)
                                col_rsids = col_data[col_data.str.match(r'^rs\d+$', na=False)]
            ...
            logger.info(f"Successfully parsed {len(df)} rsIDs from HTML {self.file_path}")
        ...
        try:
            ...
            if isinstance(rsid_column, int):
                ...
                if rsid_column >= len(df.columns):
                    raise ValueError(f"Column index {rsid_column} out of range. File has {len(df.columns)} columns.")
                ...
                if rsid_column not in df.columns:
                    raise ValueError(f"Column '{rsid_column}' not found. Available: {list(df.columns)}")
