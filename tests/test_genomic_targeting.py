...
class TestGenomicTargetingConfig:
    ...
    def test_config_manager_genomic_targeting_methods(self):
        ...
        assert config_manager.is_genomic_targeting_enabled() == True
        ...
        assert config_manager.get_genomic_targeting_default_value() == False
        assert config_manager.is_genomic_targeting_missing_file_allowed() == True
        assert config_manager.is_genomic_targeting_validation_enabled() == False
    ...
    def test_config_manager_defaults(self):
        ...
        assert config_manager.is_genomic_targeting_enabled() == False
        assert config_manager.get_genomic_targeting_file_path() == "data/manual/genomic_targeting_flags.xlsx"
        assert config_manager.get_genomic_targeting_gene_column() == "gene_symbol"
        assert config_manager.get_genomic_targeting_column() == "targeting"
        assert config_manager.get_genomic_targeting_default_value() == False
        assert config_manager.is_genomic_targeting_missing_file_allowed() == True
...
class TestBooleanConversion:
    ...
    def test_convert_to_boolean_string_values(self):
        ...
        assert convert_to_boolean("YES") == True
        assert convert_to_boolean("y") == True
        assert convert_to_boolean("Y") == True
        assert convert_to_boolean("1") == True
        assert convert_to_boolean("t") == True
        assert convert_to_boolean("T") == True
        ...
        assert convert_to_boolean("NO") == False
        assert convert_to_boolean("n") == False
        assert convert_to_boolean("N") == False
        assert convert_to_boolean("0") == False
        assert convert_to_boolean("f") == False
        assert convert_to_boolean("F") == False

    def test_convert_to_boolean_numeric_values(self):
        """Test conversion of numeric values to boolean."""
        assert convert_to_boolean(1) == True
        assert convert_to_boolean(1.0) == True
        assert convert_to_boolean(0) == False
        assert convert_to_boolean(0.0) == False
        assert convert_to_boolean(5) == True  # Non-zero numbers are True
    ...
    def test_convert_to_boolean_edge_cases(self):
        ...
        assert convert_to_boolean(None, default=True) == True
        assert convert_to_boolean(pd.NA, default=False) == False
        assert convert_to_boolean("unknown", default=True) == True
        assert convert_to_boolean("", default=False) == False
        assert convert_to_boolean(" ", default=True) == True  # Whitespace
...
class TestFileProcessing:
    ...
    def test_process_targeting_file_excel(self):
        ...
        try:
            ...
            assert len(result) == 4
            assert result["BRCA1"] == True
            assert result["BRCA2"] == True
            assert result["TP53"] == True
            assert result["APC"] == False
    ...
    def test_process_targeting_file_csv(self):
        ...
        try:
            ...
            assert len(result) == 3
            assert result["MLH1"] == True
            assert result["MSH2"] == False
            assert result["CHEK2"] == True
...
class TestFlagApplication:
    ...
    def test_apply_genomic_targeting_flags_enabled(self):
        ...
        try:
            ...
            assert brca1_row["genomic_targeting"].iloc[0] == True
            
            tp53_row = result[result["approved_symbol"] == "TP53"]
            assert tp53_row["genomic_targeting"].iloc[0] == True
            
            brca2_row = result[result["approved_symbol"] == "BRCA2"]
            assert brca2_row["genomic_targeting"].iloc[0] == False  # Default value
            
            apc_row = result[result["approved_symbol"] == "APC"]
            assert apc_row["genomic_targeting"].iloc[0] == False  # Default value
    ...
    def test_apply_genomic_targeting_flags_disabled(self):
        ...
        assert "genomic_targeting" in result.columns
        assert all(result["genomic_targeting"] == False)
...
class TestIntegration:
    ...
    def test_fetch_genomic_targeting_flags_missing_file(self):
        ...
        result = fetch_genomic_targeting_flags(config)
        assert len(result) == 0  # Should return empty dict when file missing but allowed
...
class TestEdgeCases:
    ...
    def test_process_targeting_file_with_nan_values(self):
        ...
        try:
            ...
            assert len(result) == 1  # Only BRCA1 has both valid gene symbol and targeting value
    ...
    def test_apply_flags_with_duplicate_genes(self):
        ...
        try:
            ...
            assert all(brca1_rows["genomic_targeting"] == True)
            
            brca2_rows = result[result["approved_symbol"] == "BRCA2"]
            assert all(brca2_rows["genomic_targeting"] == False)
...
class TestHTMLReportIntegration:
    ...
    def test_genomic_targeting_in_html_report_columns(self):
        ...
        table_data, available_columns, default_visible = report_gen._prepare_table_data(gene_df)
