...
class NimaGenHESTParser(BaseSNPScraper):
    ...
    def parse(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Successfully extracted {len(snps)} SNPs from NimaGen HEST panel")
        ...
        try:
            with pdfplumber.open(pdf_path) as pdf:
                for page_num, page in enumerate(pdf.pages):
                    ...
                    if "EasySeq" in text and "Human DNA Sample Identification" in text and "Targets Overview" in text:
                        ...
                        for table in tables:
                            ...
                            for row_idx, row in enumerate(table):
                                ...
                                for cell in row[1:4]:  # Check first few columns after row number
            ...
            if not target_table_found:
                logger.warning("Target table not found, extracting rsIDs from entire document")
                with pdfplumber.open(pdf_path) as pdf:
            ...
            logger.info(f"Extracted {len(unique_rsids)} unique rsIDs from NimaGen HEST PDF")
            return unique_rsids
