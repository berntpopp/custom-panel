...
class BlueprintParser(BaseParser):
    ...
    def parse(self) -> list[str]:
        ...
        try:
            ...
            for keyword in ["panel content", "genes included", "gene list", "genes tested"]:
                ...
                for element in elements:
                    ...
                    while current and search_depth < 10:
                        # Check for tables
                        next_table = current.find_next('table') if hasattr(current, 'find_next') else None
                        if next_table:
                        ...
                        # Check for lists
                        next_list = current.find_next(['ul', 'ol']) if hasattr(current, 'find_next') else None
                        if next_list:
            ...
            if not genes:
                for table in soup.find_all("table"):
                    ...
                    if any(indicator in table_text for indicator in ["GENE", "SYMBOL", "OMIM", "TRANSCRIPT"]):
            ...
            if not genes:
                gene_containers = soup.find_all("div", class_=lambda x: x and any(
            ...
            if len(unique_genes) > 0:
                logger.info(f"Extracted {len(unique_genes)} genes from Blueprint Genetics panel")
            else:
                ...
                for element in soup.find_all(["td", "li", "span"]):
                    ...
                    if 2 <= len(text) <= 10 and text.isupper():
                        ...
                        if cleaned and self.validate_gene_symbol(cleaned) and self._is_likely_gene(cleaned):
    ...
    def _is_likely_gene(self, text: str) -> bool:
        ...
        non_gene_terms = {
            ...
            "MENU", "SEARCH", "LOGIN", "LOGOUT", "SUBMIT", "CANCEL", "CLOSE",
            "NEXT", "PREV", "BACK", "HOME", "ABOUT", "CONTACT", "HELP", "FAQ",
            "NEWS", "EVENTS", "CAREERS", "PRICING", "CART", "CHECKOUT", "PAY",
            
            # Form elements
            "FIRST", "LAST", "NAME", "EMAIL", "PHONE", "ADDRESS", "CITY", "STATE",
            "ZIP", "COUNTRY", "USERNAME", "PASSWORD", "CONFIRM", "CAPTCHA",
            
            # Medical terms that aren't genes
            "TEST", "TESTS", "TESTING", "PANEL", "PANELS", "ANALYSIS", "REPORT",
            "RESULTS", "CLINICAL", "MEDICAL", "DIAGNOSTIC", "SCREENING",
            
            # Database/Reference terms
            "OMIM", "HGMD", "CLINVAR", "REFSEQ", "GENBANK", "PUBMED", "DOI",
            
            # Company/Legal terms
            "LLC", "INC", "LTD", "CORP", "COPYRIGHT", "RIGHTS", "RESERVED",
            "PRIVACY", "TERMS", "CONDITIONS", "POLICY", "DISCLAIMER",
            
            # Technical terms
            "VERSION", "UPDATE", "DOWNLOAD", "UPLOAD", "PDF", "CSV", "JSON",
            "API", "SDK", "URL", "URI", "HTTP", "HTTPS", "SSL", "TLS"
        }
        ...
        suspicious_patterns = [
            "MENU", "FIRST", "LAST", "ADD", "REMOVE", "SEARCH", "NEWS", 
            "CAREER", "CONTACT", "PRICING", "PATIENT"
        ]
        ...
        diseases = [
            "ANEMIA", "CARCINOMA", "MELANOMA", "LYMPHOMA", "NEUTROPENIA", 
            "THROMBOCYTOSIS", "SPHEROCYTOSIS", "OSTEOPETROSIS", "HEMOCHROMATOSIS",
            "ALBINISM", "ANGIOEDEMA", "CYLINDROMATOSIS", "DEAFNESS", "ELLIPSOCYTOSIS",
            "ERYTHROCYTOSIS", "ERTHYROCYTOSIS", "ERYTHROPOIETIC", "FIBROMATOSIS", 
            "MYOPATHY", "MYXOMA", "NYSTAGMUS", "OLIGONDONTIA", "PERIODONTITIS", 
            "PIEBALDISM", "PNEUMOTHORAX", "POLYPOSIS", "PROTOPORPHYRIA", 
            "SCHWANNOMATOSIS", "SCOLIOSIS", "SEIZURES", "SPASTICITY", "WARTS", 
            "SITOSTEROLEMIA", "THROMBOPHILIA", "THROMBOCYTHEMIA", "NEUTROPHILIA",
            "ACRODYSOSTOSIS", "AFIBRINOGENEMIA", "HEMOGLOBIN"
        ]
        if text in diseases:
            return False
        ...
        # Descriptive terms
        descriptive = [
            "FAMILIAL", "JUVENILE", "ADULT-ONSET", "CHILDHOOD-ONSET", "CONGENITAL",
            "MULTIPLE", "ISOLATED", "ATYPICAL", "INTRACARDIAC", "INHERITANCE",
            "FINNISH", "NORWEGIAN", "PULMONARY", "PAPILLARY", "PARATHYROID",
            "PHOTOSENSITIVE", "PLATELET-TYPE", "RECURRENT", "INFECTIONS",
            "MULTISYSTEM", "NON-HODGKIN", "GINGIVAL", "OCULOCUTANEOUS",
            "PARAGANGLIOMAS", "LYMPEHEDEMA", "VKORC1-RELATED"
        ]
